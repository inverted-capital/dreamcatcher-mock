...
import { ChevronDown, ChevronRight, Code, FileText, GitBranch, Home } from 'lucide-react';
...
const NavigationMarker: React.FC<NavigationMarkerProps> = ({ item }) => {
  ...
  const contextParts = item.context ? item.context.map(context => ({
    type: context.type,
    value: context.value
  })) : [];
  ...
  const getIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case 'MessageSquare': return <span className="text-sm">💬</span>;
      case 'Folder': return <span className="text-sm">📁</span>;
      case 'GitBranch': return <span className="text-sm">⑂</span>;
      case 'HelpCircle': return <span className="text-sm">❓</span>;
      case 'Weather': return <span className="text-sm">🌤️</span>;
      case 'Users': return <span className="text-sm">👥</span>;
      case 'User': return <span className="text-sm">👤</span>;
      case 'Lightbulb': return <span className="text-sm">💡</span>;
      case 'Settings': return <span className="text-sm">⚙️</span>;
      case 'Package': return <span className="text-sm">📦</span>;
      ...
      >
        <div className="flex items-center flex-1">
          {item.children.length > 0 && (
            <button onClick={toggleCollapse} className="mr-1 text-gray-500 hover:text-gray-700">
          ...
          <div className="flex flex-wrap items-center gap-1 ml-auto">
            {contextParts.map((part, idx) => (
              ...
              >
                ...
                {part.type === 'branch' && <GitBranch size={10} className="mr-1" />}
                {part.type === 'file' && <FileText size={10} className="mr-1" />}
                {part.value}
              </button>
            ))}
          </div>
