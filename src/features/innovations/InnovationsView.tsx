...
import { Lightbulb, Search, Filter, ArrowUp, ArrowDown, Link, ExternalLink, Plus, ChevronDown, ChevronRight } from 'lucide-react';
...
const mockInnovations: Innovation[] = [
  // Problems (formerly issues)
  {
    ...
    description: 'When the system experiences high traffic, API responses are timing out after 30 seconds.',
  ...
  },
  {
    ...
    description: 'The layout is not rendering correctly on devices with screen width below 320px.',
  ...
  },
  {
    ...
    description: 'Customer data updates are not being propagated to the CRM system in real-time.',
  ...
  },
  {
    ...
    description: 'There is a typo in the welcome email that is sent to new users.',
  ...
  // Solutions
  {
    ...
    description: 'Reduce timeout issues by implementing a connection pool that manages and reuses connections efficiently.',
    repository: 'api-server',
    status: 'proposed',
    type: 'solution',
  ...
  },
  {
    ...
    description: 'Fix mobile layout by implementing additional breakpoints and adjusting media queries for small screens.',
    repository: 'frontend-app',
    status: 'approved',
    type: 'solution',
  ...
  },
  {
    ...
    description: 'Resolve sync issues by updating the integration to use the new CRM API endpoints and authentication method.',
    repository: 'integration-service',
    status: 'proposed',
    type: 'solution',
  ...
  },
  {
    ...
    description: 'Implement exponential backoff retry pattern in the authentication service to handle intermittent failures.',
    repository: 'auth-service',
    status: 'implemented',
    type: 'solution',
  ...
  },
  {
    ...
    description: 'Update the welcome email template to fix the typo in the greeting message.',
    repository: 'notification-service',
    status: 'implemented',
    type: 'solution',
  ...
  },
  {
    ...
    description: 'Implement Redis caching for frequently accessed data to reduce database load and improve response times.',
    repository: 'api-server',
    status: 'proposed',
    type: 'solution',
    relatedItems: ['solution-1'],
...
const InnovationsView: React.FC = () => {
  ...
  const [filterType, setFilterType] = useState<'all' | 'problem' | 'solution'>('all');
      ...
      <div key={item.id} className="mb-2">
        ...
        >
          ...
          {hasChildren && !isExpanded && (
            <div className="text-xs text-blue-600 hover:text-blue-800 cursor-pointer" onClick={() => toggleExpandItem(item.id)}>
              Show {relatedItems.length} related item{relatedItems.length !== 1 ? 's' : ''}
            </div>
          )}
        </div>
              ...
              <div className="flex items-center text-gray-500">
                ...
                {item.relatedItems && item.relatedItems.length > 0 && (
                  <div className="flex items-center">
                    ...
                    <span>
                      {item.relatedItems.length} related item{item.relatedItems.length !== 1 ? 's' : ''}
                    </span>
                  </div>
                )}
              </div>
    ...
    <div className="animate-fadeIn">
          ...
          <div className="relative">
            ...
            >
              ...
              {filterType === 'all' ? 'All Types' : 
                filterType === 'problem' ? 'Problems' : 'Solutions'}
            </button>
      ...
      {filteredInnovations.length === 0 ? (
        <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
          ...
          <h2 className="text-xl font-medium text-gray-700 mb-2">No innovations found</h2>
