...
const mockInstalledNapps: Napp[] = [
  ...
  { 
    ...
    description: 'Generates documentation from code comments and project structure', 
    version: '1.0.5', 
    isEnabled: false, 
...
const mockAvailableNapps: Napp[] = [
  { 
    ...
    description: 'Manage database schemas, migrations, and queries directly from the repository', 
    version: '3.1.0', 
    isEnabled: false, 
    installDate: '' 
  },
  { 
    id: '5', 
    name: 'CI/CD Pipeline', 
    description: 'Configure and manage continuous integration and deployment workflows', 
    version: '2.2.1', 
    isEnabled: false, 
    installDate: '' 
  },
...
export const useNappsStore = create<NappsState>((set) => ({
  ...
  uninstallNapp: (id: string) => set(state => {
    const nappToUninstall = state.installedNapps.find(napp => napp.id === id);
    if (!nappToUninstall) return state;
    
    return {
      availableNapps: [...state.availableNapps, {
        ...nappToUninstall,
        isEnabled: false,
        installDate: ''
      }],
  ...
  toggleConfigPanel: (id: string) => set(state => ({
    expandedConfig: state.expandedConfig === id ? null : id
  })),
  
  runNapp: (id: string) => set(state => {
